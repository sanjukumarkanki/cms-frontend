...
const FollowupCard = (props) => {
    ...
    const updateTextArea =   async (e, bodyData) => {   
        if(bodyData.field === "time"){
            ...
            if(getTime >= hours && getTime < 18){
                ...
                setTimerError("You can't set a time before the current time and after 6 pm")
            }
        }
        else{
                const options = {
  ...
  return (
    <div className='followup-card' key={index} style={{backgroundColor : cardBgColor}}>
        <Link to={`/patient/${each.id}`} className='followup-card__name-container'>
            <p>Name :</p>
        ...
        <div className='followup-card__stage-lead-container' >
            ...
            <div className='followup-card__lead-container'>
                <label>Level:</label>
                <select value={LeadValue} onChange={(e) => onLeadSelect(e, 
                    {id : each.id, field : 'level', followupId : each.followupId})}>
        ...
        <div className='followup-card__snooze-done-container' >
            <button onClick={() => setVisible(true)} style={{color : `${each.level === "Cold" ? '#80288F' : '#fff'}`}} ><TiTick className='tick-icon' /> Done</button>
            <Dialog  visible={visible} style={{ width: '50vw' }} onHide={() => setVisible(false)}>
            <Editor style={{width : '100%'}} value={text} onTextChange={(e) => {
                if (e.htmlValue !== null) {
            ...
            <Dialog  visible={popupTimer} style={{ width: '15rem' }} onHide={() => setPopupTimer(false)}>
                <div className='timer-popup'>
                    <h3>Set The Time </h3>
                    <input type='time' style={{textAlign : 'center'}}  value={inputTimer} onChange={(e) => setInputTimer(e.target.value)}  className='timer-container' />
